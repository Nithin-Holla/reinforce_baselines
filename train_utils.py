...
def run_episode_logN(env, model, select_action=select_action_default,
	...
	while not done:
		...
		if render:
			...
			time.sleep(0.05)
		episode += [
			{"state": s, "action": action, "log_p": log_p, "reward": r, "done": done, "baseline": None, "beams": None}]
		s = s_next
		frame_iter += 1
		add_interaction()
	if render:
	...
	beam_start_states = sorted(
		list(set([(episode_length - log_basis ** i) for i in range(beam_start_freq, num_beam_start_states)] + [0])))
